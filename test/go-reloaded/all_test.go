...
func BTreeIsEqual(rootA, rootB *student.TreeNode) bool {
	...
	if ((rootA == nil) && (rootB != nil)) || ((rootA != nil) && (rootB == nil)) {
		//log.Println(rootA.Data, rootB.Data)
		//log.Println("TEETETETETETETET", rootB.Data, "PARENT", rootB.Parent.Data, rootA.Parent.Data)
		return false
	}
	if rootA.Data != rootB.Data {
		//log.Println(rootA.Data, rootB.Data)
		//log.Println("TEETETETETETETET")
		return false
	}
...
func catchCat(t *testing.T, program string, input []string, stdin bool, nativeCat bool) string {
	...
	// 	catPW, _ := cat.StdinPipe()
	// 	catPR, _ := cat.StdoutPipe()
	// 	cat.Start()
	// }
	if stdin {
...
func TestAtoi(task *testing.T) {
	...
	for _, value := range testing {
		...
		if result != value.expected {
			errorsArray = append(errorsArray, fmt.Sprintf(strings.TrimPrefix(task.Name(), "Test")+" got: %d, want: %d", result, value.expected))
			//task.Fail()
			pass = false
		}
	}
	//if pass {
	fmt.Printf("%-20s %s\n", strings.TrimPrefix(task.Name(), "Test"), status(pass))
	//}
}
...
func TestPrintCombN(task *testing.T) {
	...
	for _, value := range testing {
		...
		if result != value.expected {
			//task.Errorf
			errorsArray = append(errorsArray, fmt.Sprintf(strings.TrimPrefix(task.Name(), "Test")+" got: %s, want: %s", result, value.expected))
			//task.Fail()
			pass = false
		}
	}
		...
		if len(result) == len(value.expected) {
			for i := range result {
				if result[i] != value.expected[i] {
					errorsArray = append(errorsArray, fmt.Sprintf(strings.TrimPrefix(task.Name(), "Test")+" got: %v, want: %v", result, value.expected))
					pass = false
				}
			}
		} else {
	...
	for _, value := range testing {
		...
		for i := range value.input {
			if value.input[i] != value.expected[i] {
				errorsArray = append(errorsArray, fmt.Sprintf(strings.TrimPrefix(task.Name(), "Test")+" got: %v, want: %v", value.input, value.expected))
				pass = false
			}
		}
	}
	...
	for _, value := range testing {
		...
		for currentExpected != nil {
			if currentResult == nil || currentResult.Data != currentExpected.Data {
				errorsArray = append(errorsArray, fmt.Sprintf(strings.TrimPrefix(task.Name(), "Test")+" got: %v, want: %v", PrintList(value.input), PrintList(value.expected)))
				pass = false
				break
			}
...
func TestSortedListMerge(task *testing.T) {
	...
	for _, value := range testing {
		...
		for currentExpected != nil {
			if currentResult == nil || currentResult.Data != currentExpected.Data {
				errorsArray = append(errorsArray, fmt.Sprintf(strings.TrimPrefix(task.Name(), "Test")+" got: %v, want: %v", PrintList(result), PrintList(value.expected)))
				pass = false
				break
			}
			currentExpected = currentExpected.Next
...
func TestListRemoveIf(task *testing.T) {
	...
	for _, value := range testing {
		...
		for currentExpected != nil {
			if currentResult == nil || currentResult.Data != currentExpected.Data {
				errorsArray = append(errorsArray, fmt.Sprintf(strings.TrimPrefix(task.Name(), "Test")+" got: %v, want: %v", PrintList2(value.input), PrintList2(value.expected)))
				pass = false
				break
			}
			currentExpected = currentExpected.Next
...
func TestBTreeTransplant(task *testing.T) {
	...
	for _, value := range testing {
		...
		if !BTreeIsEqual(result, value.expected) {
			errorsArray = append(errorsArray, fmt.Sprintf(strings.TrimPrefix(task.Name(), "Test")+"\ngot: \n%vwant: \n%v", printBTree(result), printBTree(value.expected)))
			pass = false
		}
	}
	fmt.Printf("%-20s %s\n", strings.TrimPrefix(task.Name(), "Test"), status(pass))
